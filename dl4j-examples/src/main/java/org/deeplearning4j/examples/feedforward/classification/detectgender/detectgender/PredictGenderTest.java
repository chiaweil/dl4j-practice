...
public class PredictGenderTest implements Runnable {
    ...
    public void run()
    {
        try
        {
            this.filePath = new ClassPathResource("PredictGender").getFile().getAbsolutePath() + "/NewData/";
            this.possibleCharacters = " abcdefghijklmnopqrstuvwxyz";
            this.model = ModelSerializer.restoreMultiLayerNetwork(this.filePath + "PredictGender_new.zip");
        }
        catch(Exception e)
        {
